...
#' path = "./netowrk/"
#' dir.create(path)
#' result = network.2.g(ps = ps,N = 100,r.threshold=0.6,big = T,
#'                    select_layout = T,
...
network.2.g = function(
  ...
  for (layout in layouts) {
    ...
    print(layout)
    if (big == TRUE) {
      result = cor_Big_micro(ps = psi,N = 0,
                             r.threshold= r.threshold,
                             p.threshold=p.threshold,
                             method = method,
                             scale = FALSE)
      a = 2} else if(big == FALSE){
        result = corMicro (ps = psi,N = 0,
                           r.threshold= r.threshold,p.threshold=p.threshold,

#' # The algorithm imitate Gephi's network layout. while, it saves time and the calculation speed is faster
#'
#' @title Model_Gephi:The algorithm imitate Gephi's network layout. while, it saves time and the calculation speed is faster
...
Model_Gephi <- function(cor = cor,method = "cluster_fast_greedy"){
  corr = cor
  # Construct Edge File
  ...
  for (j in 1:dim(tab1)[1]) {
    ...
    if (dim(dat)[1] <= 2 |tab1$Freq[j] == tab1$Freq[dim(tab1)[1]]) {
      lacat = row.names(dat) [1:tab1$Freq[j]]
    }

    if (dim(dat)[1] > 2 & tab1$Freq[j] != tab1$Freq[dim(tab1)[1]]) {
      d <- dist(dat[,-3],method = "minkowski")
      nn = dbscan::kNN(d, k = tab1$Freq[j] , sort = TRUE, search = "linear", bucketSize = 10, splitRule = "STD", approx = 0)
      attributes(nn)
      lacat <-as.data.frame(nn$id) %>% row.names()
      lacat <- lacat[1:tab1$Freq[j]]
    }

    new.dat = dat[lacat,]
    New.locat  = netClu$ID[netClu$group == as.numeric(tab1$Var1[j] )]
